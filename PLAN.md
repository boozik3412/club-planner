...
Статус: этапы 0–14 реализованы. Подписанные автоматические обновления и выбираемые постоянные размеры опубликованы в `v0.1.2`; публичный релиз содержит installer/portable `.exe`, `.sig`, `latest.json` и SHA-256. Архитектурные высоты, `.clubplan` v2, схематичная 3D-визуализация, контекстная вставка, панорамирование ПКМ и `Ctrl+C` / `Ctrl+V` также включены в актуальную версию. Проверка установки на отдельной чистой Windows VM остаётся дополнительным внешним release-gate.
