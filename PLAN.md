...
Статус: этапы 0–13 реализованы. Публичный Windows-релиз `v0.1.0` собран проверенным GitHub Actions workflow, опубликован с installer/portable `.exe` и SHA-256, затем скачан и повторно проверен запуском, установкой и удалением. После релиза реализованы вставка предметов в центр текущей видимой области, панорамирование удержанием ПКМ и внутренний буфер выбора `Ctrl+C` / `Ctrl+V`. Архитектурные высоты, `.clubplan` v2 и схематичная 3D-визуализация включены в актуальный релиз. Проверка установки на отдельной чистой Windows VM остаётся дополнительным внешним release-gate.
