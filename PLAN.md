...
Статус: этапы 0–13 реализованы и опубликованы в `v0.1.1`. Этап 14 — подписанные автоматические обновления Windows — в работе. Публичные релизы уже содержат installer/portable `.exe` и SHA-256; архитектурные высоты, `.clubplan` v2, схематичная 3D-визуализация, контекстная вставка, панорамирование ПКМ и `Ctrl+C` / `Ctrl+V` включены в актуальную версию. Проверка установки на отдельной чистой Windows VM остаётся дополнительным внешним release-gate.
