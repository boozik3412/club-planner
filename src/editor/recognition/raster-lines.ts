...
function mergeFragments(lines: readonly DetectedLine[], minimumLengthPx: number): DetectedLine[] {
  ...
  for (const source of [...lines].sort((first, second) => {
    ...
    const matchIndex = result.findIndex((existing) => {
      ...
      return normalDistance <= 8 && start <= reference.endProjection + gapPx && reference.startProjection <= end + gapPx;
    });
    if (matchIndex < 0) {
      result.push(source);
      continue;
...
export function consolidateWallLines(
  ...
  const minimumThicknessPx = 2.5;

  candidates.forEach((first, firstIndex) => {
    if (paired.has(firstIndex)) return;
    ...
    candidates.forEach((second, secondIndex) => {
      if (secondIndex === firstIndex || paired.has(secondIndex) || !parallel(first, second)) return;
  ...
  const longSoloThreshold = Math.max(80, minimumLengthPx * 1.6, minimumDimension * 0.22);
  candidates.forEach((item, index) => {
    if (paired.has(index) || item.length < longSoloThreshold || followsImageBoundary(item.line)) return;
    result.push({
      ...item.line,
      confidence: Math.min(0.9, 0.58 + item.length / maximumDimension * 0.48),
    });
  });
  return result;
}
