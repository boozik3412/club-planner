...
Автономный русскоязычный редактор помещений и расстановки оборудования для компьютерных клубов, кафе, баров и ресторанов. Новый проект начинается с пустого листа; план можно импортировать из PDF/изображения или начертить вручную. Приложение работает в отдельном Windows-окне на Tauri 2 и не использует внешние CDN или сеть для основной работы.

## Скачать для Windows

Готовые `.exe` находятся на странице [последнего релиза](https://github.com/boozik3412/club-planner/releases/latest):

- [Club-Planner_0.2.7_x64-setup.exe](https://github.com/boozik3412/club-planner/releases/latest/download/Club-Planner_0.2.7_x64-setup.exe) — рекомендуемый установщик с автообновлением;
- [Club-Planner_0.2.7_x64-portable.exe](https://github.com/boozik3412/club-planner/releases/latest/download/Club-Planner_0.2.7_x64-portable.exe) — запуск без установки;
...
Опубликованная стабильная версия — `0.2.7`.
...
- пустой новый проект, прямоугольные помещения (`P`) и магнитная установка дверей (`D`) на архитектурные стены или предметы-перегородки; исторический обмер сохраняется только для совместимости старых проектов;
- семантические стены/двери/окна, сетка, умные привязки к стенам, объектам и совпадающим рядам, зум, панорама и поворот холста;
...
- магнитное дочерчивание прямых и настоящих дуговых стен на этапе проверки: одноразовая дуга по трём точкам без полного круга, редактируемые концы/кривизна, живые длина/угол/радиус, точный ввод, общие узлы, безопасное разделение host wall и единый Undo/Redo;
- рамочное множественное выделение распознанных прямых/дуг с `Shift` и пакетным `Delete` на шаге проверки;
...
- `src-tauri/target/release/bundle/nsis/Club Planner_0.2.7_x64-setup.exe`.
...
| Выбор / Рука / Линейка / Помещение / Дверь | `V` / `H` / `M` / `P` / `D` |
...
- [Примечания к версии 0.2.7](docs/RELEASE_NOTES_0.2.7.md)
- [Примечания к версии 0.2.6](docs/RELEASE_NOTES_0.2.6.md)
- [Примечания к версии 0.2.5](docs/RELEASE_NOTES_0.2.5.md)
